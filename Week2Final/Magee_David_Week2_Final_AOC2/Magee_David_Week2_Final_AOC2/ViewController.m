...
{
    int multiplied = multiplier.value; //Grabs the stepper value
    if (destroyerButton.enabled && cargoButton.enabled && bountyButton.enabled == true) //Runs a check to see if any buttons have been pressed.
    {
        mainLabel.text = [NSString stringWithFormat:@"Multiplier is set at %d", multiplied]; //Sets text
    } else if (destroyerButton.enabled == false) //Checks to see if destroyer is checked
    {
    ...
    } else if (cargoButton.enabled == false) //Checks to see if cargo is checked
    {
    ...
    } else if (bountyButton.enabled == false)//checks to see if bounty is checked
    {
